...
`Cutscene.Player` automatically tracks effects (e.g., fade overlay), exposes `isActive()`, `isFinished()`, and can be drawn via `player.draw(context)` after the render pass.
