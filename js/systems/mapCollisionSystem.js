...
function moveH(entity, map, value) {
    ...
    while (round(position.x, value) != desiredX) {
        if (!map.pointIsCollidingWithWall(position.x + sign + modifier, position.y) && 
...
ECS.Helpers.collidingWithMap = function(entity, map) {
    ...
    return map.pointIsCollidingWithWall(x, y)
        ...
        || map.pointIsCollidingWithWall(x + dimensions.width - 1, y + dimensions.height - 1);
}
...
ECS.Helpers.collidingWithBlockType = function(entity, map, type) {
    ...
    return map.pointIsCollidingWithType(x, y, type)
        ...
        || map.pointIsCollidingWithType(x + dimensions.width - 1, y + dimensions.height - 1, type);
}
...
ECS.Helpers.isOffMap = function(entity, map) {
    ...
    return position.x < 0 - dimensions.width || 
