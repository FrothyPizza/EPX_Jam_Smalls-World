...
ECS.Systems.playerPhysicsSystem = function(entities) {
    Object.values(entities).forEach(entity => {
        ...
        if (collision.bottomHit && Math.abs(velocity.x) < 0.5) { velocity.x = 0; }
        if (Math.abs(velocity.x) < movement.xVelLowerThreshold) { velocity.x = 0; }
    });
}
...
ECS.Systems.playerJumpSystem = function(entities, map) {
    Object.values(entities).forEach(entity => {
        ...
        } else if (!jump.hasCutJumpVelocity && jump.jumpHoldTimer.getTime() < jump.maxJumpHoldTime) {
            jump.jumpHoldTimer.add(10000);
...
ECS.Systems.playerMovementSystem = function(entities) {
    Object.values(entities).forEach(entity => {
        if (!entity.has('PlayerMovement', 'Velocity', 'AnimatedSprite', 'Position')) return;
        ...
        let moved = false;
        // Move left
        if (Inputs.left && Math.abs(velocity.x) < movement.speed && velocity.x <= 0.5) {
        ...
        // Move right
        if (Inputs.right && Math.abs(velocity.x) < movement.speed && velocity.x >= -0.5) {
            velocity.x = movement.speed;
            sprite.direction = 1;
            moved = true;
        }
...
ECS.Systems.playerFlyingSystem = function(entities) {
    Object.values(entities).forEach(entity => {
        if (!entity.has('PlayerFlying', 'Velocity', 'Gravity', 'PlayerMovement')) return;
...
ECS.Systems.playerSpikeDamageSystem = function(entities, map) {
    Object.values(entities).forEach(entity => {
        ...
        if (map.pointIsCollidingWithSpikes(position.x, position.y) ||
            map.pointIsCollidingWithSpikes(position.x + dimensions.width - 1, position.y) ||
            map.pointIsCollidingWithSpikes(position.x, position.y + dimensions.height - 1) ||
            map.pointIsCollidingWithSpikes(position.x + dimensions.width - 1, position.y + dimensions.height - 1)) {
...
ECS.Systems.playerOffMapSystem = function(entities, map) {
    Object.values(entities).forEach(entity => {
        if (!entity.has('Position', 'PlayerSpawn', 'PlayerLives', 'Velocity')) return;
        ...
        if (position.y > map.height * map.tileheight - 6) {
            if (entity.PlayerLives.lives > 0 && spawn.lastGroundedPositions.length > 0) {
...
ECS.Systems.playerInvincibilitySystem = function(entities) {
    Object.values(entities).forEach(entity => {
        ...
        if (invincibility.isInvincible && 
            invincibility.invincibilityTimer.getTime() > invincibility.invincibilityDuration) {
...
ECS.Systems.playerStateMachineSystem = function(entities) {
    Object.values(entities).forEach(entity => {
        ...
        } else if (!collision.bottomHit && ((collision.rightHit && Inputs.right) || (collision.leftHit && Inputs.left))) {
            state.state = PLAYER_STATES.WALL_SLIDING;
        } else if (entity.has('PlayerGlide') && !collision.bottomHit && Inputs.jump && 
                   velocity.y > 0 && velocity.y === entity.PlayerGlide.glideFallSpeed) {
